        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                for id, lm in enumerate(hand_landmarks.landmark):
                    ...
                    # cv2.line(image, (int(center_x*w), int(center_y*h)), (int((center_x+lambda_n*a/length)*w), int((center_y+lambda_n*b/length)*h)), (0, 0, 255), 2)
                    ...
                    # top_00 = T_scale.dot(np.concatenate((T_trans_part, np.concatenate((np.array([[lambda_n, lambda_n, lambda_n]]).T + (1/scale_factor-1)*center.T, np.array([[1]])))), axis=1).dot(np.array([[0], [0], [0], [1]])))))
                    ...
                    # bottom_11 = bottom_11 + center
                    cv2.circle(image, (int((center_x+bottom_11[0])*w), int((center_y+bottom_11[1])*h)), 5, (0, 0, 255), -1)
                    # cv2.line(image, (int(thumb_tip_x*w), int(thumb_tip_y*h)), (int(index_tip_x*w), int(index_tip_y*h)), (0, 0, 255), 2)
